...
import SignIn from "./signInFurn";
import SignOut from "./signOutFurn";
import Products from "./productsFurn";
import Navbar from "./navbarFurn";
import Cart from "./cartFurn";
import NewProduct from "./newProductFurn";
class FurnMain extends Component {
  state = {
    user: "",
    cart: [],
    products: [
      ...
      },
      {
        ...
        category: "Drawing",
        desc: [
          "Two Seater Sofa in Blue Colour",
          "Assembly by Skilled Carpenters",
        ],
        img:
  ...
  componentDidMount() {
    const response = auth.getCurrentUser();
    const user = { username: response.user, role: response.role };
    this.setState({ user });
  }
  handleCart = (cart) => {
    console.log(cart);
  ...
  handleNewProduct = (product) => {
    ...
    let index = products.findIndex(
      (prod) => prod.prodCode === product.prodCode
    );
    console.log(index);
    if (index !== -1) {
  ...
  render() {
    let { user, cart, products } = this.state;
    return (
      <React.Fragment>
        <Navbar user={user} />
        <div>
          <Switch>
            {user.role === "admin" ? (
              <Route
                path="/products/:category/:prodCode/edit"
                render={(props) => (
                  <NewProduct
                    {...props}
                    products={products}
